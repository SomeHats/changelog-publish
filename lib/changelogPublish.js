...
function getLastVersion(
  ...
  | {| isValid: true, version: string | null |}
  | {| isValid: false, message: string |} */ {
  const lastVersionHeading = nodes.find(
...
function changelogPublish(
  ...
  const lastVersionResult = getLastVersion(
    ast.children.slice(unreleasedHeadingIndex + 1)
  );
  ...
  const versionLinkUrl = lastVersion
    ? `${repoUrl}/compare/v${lastVersion}...v${version}`
    : `${repoUrl}/releases/tag/v${version}`;
  const unreleasedLinkUrl = `${repoUrl}/compare/v${version}...HEAD`;
